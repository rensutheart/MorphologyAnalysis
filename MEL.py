    ...
    for label in labeledStack:
        ...
        for s in label:
            if (np.sum(s) != 0):
                ...
                # print(s.shape)
                # edges = tf.image.sobel_edges(tf.convert_to_tensor(s))[0]
...
def getHalfWayPointBetweenLabels(labeledStackF1, fromIndex, toIndex, plot=False):
    #    print("\nFrom {} to {}".format(fromIndex, toIndex))
    ...
    if (plot):
        ...
        plt.show()
        # plt.savefig("{}_{}.png".format(fromIndex, toIndex))
        # plt.close(fig)
        plt.close('all')
...
#        pass3D = labeledStack[labelIndex]
        ...
        '''
        #labeledStackKernel[labelIndex] = labeledStack[labelIndex] # NO GAUSSIAN BLUR
...
def compareOverlapV3(labeledStackF1, labeledStackF2):
    print("\ncompareOverlapV3")
    startTime = time()
    ...
    for sl in range(0, labeledStackF1.shape[1]):
        for f1LabIndex in range(0, labeledStackF1.shape[0]):
            volume = np.sum(
                np.sum(np.multiply(labeledStackF1[f1LabIndex, sl, :, :], labeledStackF2[:, sl, :, :]), axis=2), axis=1)
            volume_overlap_F1_to_F2[f1LabIndex] = np.add(volume, volume_overlap_F1_to_F2[
                f1LabIndex])  # don't erase with each slice iteration

        print(" {} ".format(sl), end='')

    print("\ncompareOverlapV3 Time: ", time() - startTime)
...
def linkStatusPerLabel(filteredBinaryF1, filteredBinaryF2, overlapF1_to_F2, overlapF2_to_F1, listAssociatedLabelsF1,
    ...
    for F1_label in range(0, listAssociatedLabelsF1.shape[0]):
        ...
        #totalVols_F1.append(labelVols_F1)
    ...
    for F2_label in range(0, listAssociatedLabelsF2.shape[0]):
        ...
        #totalVols_F2.append(labelVols_F2)
            ...
            for otherLabelF1_index in range(0, withinAssociatedLabelsF1[F1_i].shape[0]):  # associated label
                ...
                if (F1_i != withinAssociatedLabelsF1[F1_i][otherLabelF1_index]): # since can't fuse to self
                    ...
                    if (labelDistanceF1[F1_i][otherLabelF1_index] < distanceThreshold):  # some labels are not really associated due to large distance
                        ...
                        #HWP = np.around(withinAssociatedLabelsF1_HWP[F1_i][otherLabelF1_index]).astype(np.uint16)
                        #if (filteredBinaryF1[HWP[0], HWP[1], HWP[2]] == 1):
                        if (checkStructuresInBetween(filteredBinaryF1, vector_F1[F1_i][otherLabelF1_index])):
                            ...
                            for match in matchingLabelsInF2:
                                ...
                                #x.append(totalPerc_F1[F1_i][match_index_1])
                                #x.append(totalPerc_F1[otherLabelF1][match_index_2])
            ...
            for otherLabelF2_index in range(0, withinAssociatedLabelsF2[F2_i].shape[0]):  # associated label
                ...
                if (F2_i != withinAssociatedLabelsF2[F2_i][otherLabelF2_index]):  # since can't fragment to self
                    ...
                    if (labelDistanceF2[F2_i][otherLabelF2_index] < distanceThreshold):  # some labels are not really associated due to large distance
                        # check if there is a structure inbetween... to remove any remaining accidental mataches. Crudely look at the HWP
                        #HWP = np.around(withinAssociatedLabelsF2_HWP[F2_i][otherLabelF2_index]).astype(np.uint16)
                        #if (filteredBinaryF2[HWP[0], HWP[1], HWP[2]] == 1):
...
def generateGradientImage(Frame1, CoM_F1, withinChosenStatusF1_Fuse, withinChosenStatusF2_Fragment,
    ...
    # 5 layers
    # red_kernel_3D = np.stack((red_kernel*0.1,red_kernel*0.5, red_kernel, red_kernel*0.5, red_kernel*0.1))
    # green_kernel_3D = np.stack((green_kernel*0.1, green_kernel*0.5, green_kernel, green_kernel*0.5, green_kernel*0.1))
    # blue_kernel_3D = np.stack((blue_kernel*0.1, blue_kernel*0.5, blue_kernel, blue_kernel*0.5, blue_kernel*0.1))
    # if False:
    #     plt.subplot(131)
...
def showMesh(filteredBinary, labels, locations, dupLabels, dupLocations, zScale, xyScale):
    ...
    for fuseFragDep in range(0, 3):
        ...
        for index in range(0, len(labels[fuseFragDep])):
            ...
            # print(np.array([locations[fuseFragDep][index][0]*zScale, locations[fuseFragDep][index][1] * xyScale, locations[fuseFragDep][index][2] * xyScale]))
    ...
    #print(sc.camera.fov)
    #sc.camera.fov = sc.camera.fov * 1.2
    sc.show()

def paperPreProc(imageStack):
    scaleFactor = 2
    tophatBallSize = 5

    # contrast stretching
    p_min = np.percentile(imageStack, 0)
    p_max = np.percentile(imageStack, 100)
    imageStack = rescale_intensity(imageStack, in_range=(p_min, p_max))

    # Upscale
    imList = []
    for im in imageStack:
        imList.append(rescale(im, scaleFactor))

    imageStack = np.array(imList)

    # Top-hat
    imageStack = white_tophat(imageStack, ball(tophatBallSize))
...
def testPaper():
    frame1Path = "E:\\Dropbox\\Masters\\Report\\My Papers\\Paper4\\plos-latex-template\\TestFrame1.png"
    frame2Path = "E:\\Dropbox\\Masters\\Report\\My Papers\\Paper4\\plos-latex-template\\TestFrame2.png"
    blankFramePath = "E:\\Dropbox\\Masters\\Report\\My Papers\\Paper4\\plos-latex-template\\blankFrame.png"
    outputFolder = "E:\\Dropbox\\Masters\\Report\\My Papers\\Paper4\\plos-latex-template\\"
    folder = "1"
    frameText = "1"

    tFrame1 = io.imread(frame1Path)
    tFrame2 = io.imread(frame2Path)
    blankFrame = io.imread(blankFramePath)

    tFrame1 = np.stack((blankFrame, tFrame1, blankFrame))
    tFrame2 = np.stack((blankFrame, tFrame2, blankFrame))

    tFrame1 = paperPreProc(tFrame1)
    tFrame2 = paperPreProc(tFrame2)

    print("\nFRAME 1")
    Frame1 = tFrame1
    filteredBinaryF1, stackLabelsF1, numLabelsF1 = Morphology.labelStack(Frame1)
    ...
    print("{} labels detected in Frame 1".format(numLabelsF1))
    labeledStackF1 =  Morphology.stack3DTo4D(stackLabelsF1, numLabelsF1)
    filteredLabeledStackF1 = gaussianFilter(labeledStackF1, sigma2D=5)

    print("\nFRAME 2")
    Frame2 = tFrame2
    filteredBinaryF2, stackLabelsF2, numLabelsF2 =  Morphology.labelStack(Frame2)
    print("{} labels detected in Frame 2".format(numLabelsF2))
    labeledStackF2 =  Morphology.stack3DTo4D(stackLabelsF2, numLabelsF2)
    filteredLabeledStackF2 = gaussianFilter(labeledStackF2, sigma2D=5)

    # NB!!! REMOVE THIS
    # filteredLabeledStackF2 = labeledStackF2
    ...
    overlapF1_to_F2 = compareOverlapV3(filteredLabeledStackF1, filteredLabeledStackF2)
    overlapF2_to_F1 = overlapF1_to_F2.T

    # This has a filtering effect
    overlapF1_to_F2[np.where(overlapF1_to_F2 < 10)] = 0
    ...
    withinAssociatedLabelsF1_HWP, labelDistanceF1, vector_F1 = getAllHalfWayPoints(cannyLabeledStackF1, withinAssociatedLabelsF1)
    withinAssociatedLabelsF2_HWP, labelDistanceF2, vector_F2 = getAllHalfWayPoints(cannyLabeledStackF2, withinAssociatedLabelsF2)

    # determine link status
    withinChosenStatusF1_Fuse, withinChosenStatusF2_Fragment = linkStatusPerLabel(filteredBinaryF1, filteredBinaryF2,
                                                                                  overlapF1_to_F2, overlapF2_to_F1,
                                                                                  listAssociatedLabelsF1,
                                                                                  listAssociatedLabelsF2,
                                                                                  withinAssociatedLabelsF1,
                                                                                  withinAssociatedLabelsF2,
    ...
    (outputStack, outcomes, locations, labels, dupOutcomes, depLocations, depLabels) = generateGradientImage(Frame1, CoM_F1, withinChosenStatusF1_Fuse,
                                                               withinChosenStatusF2_Fragment, withinAssociatedLabelsF1,
                                                               withinAssociatedLabelsF2,
                                                               withinAssociatedLabelsF1_HWP,
                                                               withinAssociatedLabelsF2_HWP)

    Frame1RGB = np.stack((Frame1, Frame1, Frame1), axis=3)

    outputStack += np.maximum(Frame1RGB / 2, outputStack)
    outputStack = np.clip(outputStack, 0, 1)
    # fig = plt.figure()
    # plt.imshow(outputStack[1])

    # fig = plt.figure(figsize=(5,5))
    # plt.imshow(outputStack[0])

    frameOutputPath = "{}{}/".format(outputFolder, folder)
    if not os.path.exists(frameOutputPath):
        os.makedirs(frameOutputPath)

    io.imsave("{}{}.tif".format(frameOutputPath, frameText), (outputStack * 255).astype(np.uint8))

    outputImagePanel(frameOutputPath, frameText, folder, Frame1, Frame2, outputStack, filteredBinaryF1,
                     filteredBinaryF2)

    # plt.imshow(np.max(labeledStackF1[16], axis=0) +np.max(labeledStackF1[21], axis=0)*0.5 + np.max(labeledStackF1[8], axis=0)*0.25)

    # plt.hist(x, bins='auto')
    # plt.title("Time index = " + frameText +  "  Skip Frames = " + str(1))
    # plt.show()

    print("\nTime index run time: ", time() - 1)

# how many time frames to skip between comparisons. 0 is the lowest (means consecutive frames)
# def runMELBatchFolders(sourceFolder, outputFolder, startFolderNum, totalFolders, startFrameNum = 0, skipFrames=0):
#   folderOutcomeCounts = []
#     for folder in range(startFolderNum, totalFolders + 1):
#         # timelapse... load timelpase
#         folderOutcomeCounts.append(runMELBatch(timelapse, outputFolder, startFolderNum, skipFrames))
#   # print("FOLDER OUTCOMES: ", folderOutcomeCounts)
#
#   if not os.path.exists(frameOutputPath):
#        os.makedirs(frameOutputPath)
#
#   outF = open("myOutFile.txt", "w")
#   for line in folderOutcomeCounts:
...
def runMELBatch(timelapse, outputFolder, startFrameNum=0, skipFrames=0):
    eventCounts = []
    folderLocations =[]
    outputStacks = []
    folderLabels = []

    for timeIndex in range(startFrameNum, len(timelapse) + 1):
        print("TimeIndex: {}".format(timeIndex))

        frame1Stack = timelapse[timeIndex]
        frame2Stack = timelapse[timeIndex + skipFrames + 1]

        frameText = str(timeIndex)
        if (timeIndex < 10):
            frameText = "0" + frameText

        (outputStack, numEvents, locations, labels) = runMEL(frame1Stack, frame2Stack, frameText)

        print("OUTCOMES: (Fuse, Frag, Depolarize)", numEvents)
        eventCounts.append(numEvents)

        io.imsave("{}{}.tif".format(outputFolder, frameText), (outputStack * 255).astype(np.uint8))

        # outputImagePanel(outputFolder, frameText, folder, Frame1, Frame2, outputStack, filteredBinaryF1,
        #                  filteredBinaryF2)

        folderLocations.append(locations)

        outputStacks.append(outputStack)

        folderLabels.append(labels)

    np.save("{}Locations/Array".format(outputFolder), np.array(folderLocations))

    outcomeFile = open("{}Outcomes/{}.txt".format(outputFolder, "Summary"), "w")
    for line in eventCounts:
        # write line to output file
        outcomeFile.write(str(line))
        outcomeFile.write("\n")
    outcomeFile.close()

    return (outputStacks, eventCounts, folderLocations, folderLabels)
